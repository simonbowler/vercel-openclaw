...
| `/api/cron/watchdog` | Runs daily via Vercel Cron (Hobby-compatible default). Health-checks running sandboxes, repairs stuck states, and **wakes stopped sandboxes when OpenClaw cron jobs are due**. Pro users can increase frequency to `*/5 * * * *` in `vercel.json`. |
