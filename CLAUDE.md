...
| `OPENCLAW_PACKAGE_SPEC` | All environments | Optional locally, recommended on Vercel. When unset, the runtime falls back to a pinned known-good version (currently `openclaw@2026.4.11`). On Vercel deployments, the deployment contract **warns** — it does not fail — when unset or unpinned. Pin to an exact version like `openclaw@1.2.3` for deterministic sandbox resumes. |
